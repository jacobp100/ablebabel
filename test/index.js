...
function removeUnusedKeys(node) {
  if (_.isArray(node)) {
    return _.map(node, removeUnusedKeys);
  } else if (!_.isPlainObject(node)) {
    return node;
  }

  const keysToExcludeFromNode = _([
    keysToExclude['*'],
    keysToExclude[node.type],
  ])
    .compact()
    .flatten()
    .value();
  ...
  return _(node)
    .omit(keysToExcludeFromNode)
    .mapValues(removeUnusedKeys)
    .value();
}
...
function runTest(test) {
  ...
  if (opts.throws) {
    ...
    var expected = removeUnusedKeys(JSON.parse(test.expect.code));
